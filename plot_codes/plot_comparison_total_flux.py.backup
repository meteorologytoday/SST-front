...
for i, input_dir in enumerate(args.input_dirs):
    ...
    #print(ds.HFX)
    #print("HFX = ", ds.HFX.mean(dim='west_east', skipna=True).to_numpy())
    #.mean(dim=['XLONG', 'XLONG_U'])
...
time_fmt = "%Y/%m/%d %Hh"
fig.suptitle("Time: %s ~ %s\nAverage: %d ~ %d km" % (time_beg.astype(datetime.datetime).strftime(time_fmt), time_end.astype(datetime.datetime).strftime(time_fmt), args.x_rng[0], args.x_rng[1]))
